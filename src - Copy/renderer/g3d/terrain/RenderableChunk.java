...
//        IntArrayStack indices = new IntArrayStack(quads * 6);
    ...
    public void setHeights(BiFunction<Integer, Integer, Float> heightFunc) {
        float[][] heightMap = mapToArray(heightFunc, width + 1, height + 1);
//        FloatArrayStack stack = new FloatArrayStack(quads * 4 * 4);
//        FloatBuffer buffer = BufferUtils.createFloatBuffer(quads * 4 * 4);
        float[] arr = new float[quads * 4 * 4];
        int ptr = 0;

        for (int x = 0; x < width; x++) {
            for (int z = 0; z < height; z++) {
//                stack.put(heightMap[x][z], 1, 0, 0);
//                stack.put(heightMap[x + 1][z], 1, 1, 0);
//                stack.put(heightMap[x][z + 1], 0, 0, 1);
//                stack.put(heightMap[x + 1][z + 1], 0, 1, 1);
                Vector3f normal;
...
//                buffer.put(new float[]{heightMap[x][z], 1, 0, 0});
//                buffer.put(new float[]{heightMap[x + 1][z], 1, 1, 0});
//                buffer.put(new float[]{heightMap[x][z + 1], 0, 0, 1});
//                buffer.put(new float[]{heightMap[x + 1][z + 1], 0, 1, 1});
            }
        }
        vao.bind();
